...
class PGAgent(BaseAgent):
    ...
    def train(self, observations, actions, rewards_list, next_observations, terminals):
        ...
        for step in range(self.agent_params['multiple_steps']):
    ...
    def calculate_q_vals(self, rewards_list):
        ...
        if not self.reward_to_go:
            ...
            for n in range(N):
                q_values = np.concatenate((q_values, self._discounted_return(rewards_list[n])))

        # Case 2: reward-to-go PG
        # Estimate Q^{pi}(s_t, a_t) by the discounted sum of rewards starting from t
        else:
            for n in range(N):
                q_values = np.concatenate((q_values, self._discounted_cumsum(rewards_list[n])))
    ...
    def _discounted_return(self, rewards):
        ...
        T = len(rewards)
        discounted_return = np.sum(np.power(self.gamma, np.arange(T)) * np.array(rewards))
        discounted_returns = np.full((T,), discounted_return)
    ...
    def _discounted_cumsum(self, rewards):
        ...
        r = np.array(rewards)
        for t in range(T):
            discounted_cumsums[t] = np.sum(np.power(self.gamma, np.arange(0, T - t)) * r[t:])
